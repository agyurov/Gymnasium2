...
```{r}
m2 = clm(q7.1_editedprivacy ~ q2.1_nocare + q3.1_gender + q5.1_digedu,data=dfu,ink = "logit")
...
new.dfu$pred_prob = predict(m2,new.dfu)
oddz = outer(new.dfu$pred_prob,new.dfu$pred_prob,'/')
oddz = round(oddz,2)
diag(oddz) = ""
oddz[lower.tri(oddz)] = ""
...
```{r}
m2 = clm(q7.1_editedprivacy ~ (q2.1_nocare + q3.1_gender + q5.1_digedu)^2,data=dfu,link = "cauchit")
m2 = step(m2,test="Chisq",trace=0)
kable(summary(m2)$coefficients,digits=2,caption="With interactions...")
new.dfu = expand.grid(q2.1_nocare=levels(dfu$q2.1_nocare),q3.1_gender=levels(dfu$q3.1_gender),
                      q5.1_digedu=levels(dfu$q5.1_digedu))
new.dfu$pred_prob = predict(m2,newdata=new.dfu)
